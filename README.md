...
### ビルド済みバイナリを使う（Rust 不要）

[Releases](https://github.com/WatanabeYuito21/smart-cd/releases/latest) から環境に合ったファイルをダウンロードしてください:

| OS | ファイル |
|---|---|
| Linux x86_64 | `smart-cd-linux-x86_64.tar.gz` |
| macOS Intel | `smart-cd-macos-x86_64.tar.gz` |
| macOS Apple Silicon | `smart-cd-macos-aarch64.tar.gz` |
| Windows x86_64 | `smart-cd-windows-x86_64.zip` |

**Linux / macOS（手動）:**

```bash
tar xzf smart-cd-linux-x86_64.tar.gz        # macOS の場合はファイル名を変更
mkdir -p ~/.local/bin
mv smart-cd ~/.local/bin/
```

`~/.local/bin` が PATH に含まれていない場合はシェルのプロファイルに追加してください:
...
```bash
curl -sL https://raw.githubusercontent.com/WatanabeYuito21/smart-cd/main/install.sh | sh
```

**Windows:**

zip を展開して `smart-cd.exe` を PATH の通ったフォルダに置いてください。

### Cargo でインストールする

Rust / Cargo **1.85 以上** が必要です（edition 2024 を使用しているため）。

```bash
cargo install --path .
```

Cargo のバージョンが古い場合は `rustup update` で更新できます。

## Update

### ビルド済みバイナリの場合

インストールスクリプトを再実行するだけで最新版に更新されます:
...
TUI 上部の検索プロンプトに文字を入力すると候補をリアルタイムで絞り込めます。

| キー | 動作 |
|---|---|
| 文字入力 | 検索クエリに追記してリアルタイムフィルタ |
| `Backspace` | クエリの末尾を削除 |
| `j` / `↓` / `Ctrl+n` | 下へ |
| `k` / `↑` / `Ctrl+p` | 上へ |
| `g` | 先頭へ |
